...
class NonCausalOrdering {
public:
	NonCausalOrdering(int port, int id, char *msg);
	void init();
	...
	static void *senderServiceHelper(void *context){
		return ((NonCausalOrdering *)context)->handleSenderService();
	}


	static void *recvServiceHelper(void *context){
		return ((NonCausalOrdering *)context)->handleRecvService();
	}
private:
