...
void BankServer::do_action(char * data, int clientSocket){
	...
	int status = read(clientSocket,commitStatus,sizeof(commitStatus));
	_logger -> info("Commit status is: {}",commitStatus);

	char transDetails[256];
	bzero(transDetails, 256);
	if((strcmp(commitStatus,"ABORT"))!=0){
		int receive3 = read(clientSocket,transDetails,sizeof(transDetails));
		_logger -> info("Details received is: {}",transDetails);
	}else{
 ...
 */
    ...
    fprintf(stderr, "    -h file         	    Host name for the Front End Server\n");
    exit(EINVAL);
}
...
int main(int argc, char **argv) {
	...
	sinks.push_back(std::make_shared<spdlog::sinks::rotating_file_sink_mt>("./logs/FrontEndServer.txt",1024 * 1024 * 50, 10, true));
	auto combined_logger = std::make_shared<spdlog::logger>("FrontEndServer", begin(sinks), end(sinks));
